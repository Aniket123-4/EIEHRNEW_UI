...
const PatientDetailsCommon_BILLING = React.forwardRef((props: any) => {
...
// ✅ 1. initial patient search (as it is)
useEffect(() => {
...
// ✅ 2. initial load (API)
useEffect(() => {
  fetch("http://localhost:7285/api/Common/live-queue")
    .then(res => res.json())
    .then(data => {
      ...
      if (Array.isArray(data)) {
        ...
        setLivePatients(data.result);
      } else if (Array.isArray(data.data)) {
        setLivePatients(data.data);
      } else {
        setLivePatients([]);
      }
    })
...
useEffect(() => {
  const connection = new signalR.HubConnectionBuilder()
    .withUrl("http://localhost:7285/billingHub") // ❌ no credentials
...
// const handleLiveClick = (item: any) => {
    
//   onFinishPatForm({ patientNo: item.patientNo });

//   form.setFieldsValue({
//     patientNo: item.patientNo
//   });
// };
const handleLiveClick = async (item: any) => {

  const res = await requestFnGetPatientSearch({
    patientNo: "",
  ...
  const match = res?.result?.find(
    (x: any) => x.patientName === item.patientName
  );

  if (match) {
    form.setFieldsValue({
      Search: match.patientName
    });

    onChange(match.patientNo);
  }
};
const removeLiveCard = (index: number) => {
  setLivePatients(prev => prev.filter((_, i) => i !== index));
};
    ...
    <Col key={index}>
      ...
      >
        ...
        <div style={{ textAlign: "center" }}>
          <Avatar
            size={60}
            src={item?.photo ? `data:image/png;base64,${item.photo}` : null}
          >
            No Image
          </Avatar>
        </div>
        ...
        <div style={{ marginTop: 10, textAlign: "center" }}>
          ...
          <br />
          <span style={{ color: "#555" }}>{item.patientNo}</span>
        </div>
      </Card>
    </Col>
  ))}
...
export default PatientDetailsCommon_BILLING;
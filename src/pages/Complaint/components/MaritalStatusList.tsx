...
import { requestGetCivilStatus } from '@/services/apiRequest/dropdowns';
import { Table, Tag } from 'antd';
import type { ColumnType } from 'antd/es/table';
import Highlighter from 'react-highlight-words';
import { FilterConfirmProps } from 'antd/es/table/interface';

interface MaritalDataType {
...
const MaritalStatusList = ({ refresh, editRecord }: any) => {
    ...
    const [maritalList, setMaritalList] = useState<MaritalDataType[]>([]);
    ...
    const getMaritalList = async () => {
        ...
        if (res.isSuccess == true && res.result.length > 0) {
            const dataWithKeys = res.result.map((item: any, index: number) => ({
                ...item,
                key: item.civilStatusID || `marital_${index}`,
                isActive: true // Default since API doesn't return isActive
            }));
            setMaritalList(dataWithKeys);
        }
        setLoading(false);
    };
    ...
    const handleSearch = (
        ...
        dataIndex: keyof MaritalDataType,
    ) => {
        confirm();
    ...
    const getColumnSearchProps = (dataIndex: keyof MaritalDataType): ColumnType<MaritalDataType> => ({
    ...
    const columns = [
        {
            ...
            key: 'civilStatusName',
            width: '40%',
            ...getColumnSearchProps('civilStatusName'),
        },
        {
            title: 'Status Code',
            dataIndex: 'civilStatusCode',
            key: 'civilStatusCode',
            width: '30%',
        },
        // {

...
import { requestGetGender } from '@/services/apiRequest/dropdowns';
...
const GenderList = ({ refresh, editRecord }: any) => {
    ...
    const [genderList, setGenderList] = useState<GenderDataType[]>([]);
    ...
    const getGenderList = async () => {
        ...
        if (res.isSuccess == true && res.data.length > 0) {
            const dataWithKeys = res.data.map((item: any, index: number) => ({
                ...item,
                key: item.genderID || `gender_${index}`,
                isActive: true // Default since API doesn't return isActive
            }));
            setGenderList(dataWithKeys);
        }
        setLoading(false);
    };
    ...
    const handleSearch = (
        ...
        dataIndex: keyof GenderDataType,
    ) => {
        confirm();
    ...
    const getColumnSearchProps = (dataIndex: keyof GenderDataType): ColumnType<GenderDataType> => ({
    ...
    const columns = [
        {
            ...
            key: 'genderName',
            width: '40%',
            ...getColumnSearchProps('genderName'),
        },
        {
            title: 'Gender Code',
